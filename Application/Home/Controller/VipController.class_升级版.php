...
//统计该用户已发售卡数
		...
		if($_SESSION['level']<10){
			...
			$querys = $wj.$sql;
			$viplock = $cfg->where($querys)->order('sid desc')->limit(0,15)->select();
...
//用户更改资料
	...
	public function vipsearch(){
		...
		$data = $conn->where($where)->select();
			...
			$page->setConfig('theme','%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END% 第 '.I('p',1).' 页/共 %TOTAL_PAGE% 页 ( '.$pagecount.' 条/页 共 %TOTAL_ROW% 条)');
			$show = $page->show();
			$lists = $conn->join("LEFT JOIN tp_user as u ON tp_cart.uid = u.id")->where($where)->order('tp_cart.uid,tp_cart.id desc')->field("u.usercname,u.card,tp_cart.id,tp_cart.uid,tp_cart.sid,tp_cart.username,tp_cart.startime,tp_cart.endtime,tp_cart.erweima,tp_cart.jihuotime,tp_cart.lock")->limit($page->firstRow.','.$page->listRows)->order('sid asc')->select();
			$this->assign('lists',$lists);
			$this->assign('page',$show);
	...
	public function viplists(){
		...
		$conn = M('cart');
			...
			$page->setConfig('theme','%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END% 第 '.I('p',1).' 页/共 %TOTAL_PAGE% 页 ( '.$pagecount.' 条/页 共 %TOTAL_ROW% 条)');
			$show = $page->show();
			$lists = $conn->join("LEFT JOIN tp_user as u ON tp_cart.uid = u.id")->field("u.usercname,u.card,tp_cart.id,tp_cart.uid,tp_cart.sid,tp_cart.username,tp_cart.startime,tp_cart.endtime,tp_cart.erweima,tp_cart.jihuotime,tp_cart.lock")->order('sid asc')->limit($page->firstRow.','.$page->listRows)->select();
			$this->assign('lists',$lists);
			$this->assign('page',$show);
			//$this->display();
	...
	public function loglists(){
		...
		$conn = M('log');
			...
			$tongji = $conn->field("shopname,count(username) as gnum")->where('`shopname`!="" and shopname!="测试专用"')->group("shopname")->select();
