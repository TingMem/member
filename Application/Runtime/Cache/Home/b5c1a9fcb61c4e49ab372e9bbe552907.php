  ...
  <div class="container">
    <div class="row clearfix">
      <div class="col-md-12 column">
        <form class="form-horizontal" role="form" id="form1" name="form1" method="post" action="<?php echo U('StoreProductAttr/AttrEdit',array('id'=>$ata['id']));?>">
          <div class="form-group">
            <label for="attrname" class="col-sm-2 control-label">属性名</label>
            <div class="col-sm-3">
              <input name="attr_name" class="form-control" id="attrname" value="<?php echo ($ata["attr_name"]); ?>" type="text" />
            </div>
          </div>
          <div class="form-group">
            ...
            <div class="col-sm-2">
              <select name="text_type" id="text_type" class="form-control">
                ...
                <option <?php if($ata["attr_text_type"] == "list"): ?>selected<?php endif; ?> value="list">下拉菜单</option>
                <option <?php if($ata["attr_text_type"] == "input"): ?>selected<?php endif; ?> value="input">单行文本</option>
              </select>
            </div>
          </div>
          <div class="form-group">
            ...
            <div class="col-sm-2">
              <select name="attr_status" id="attr_status" class="form-control">
                <option <?php if($ata["attr_status"] == 1): ?>selected<?php endif; ?> value="1">是</option>
                <option <?php if($ata["attr_status"] == 0): ?>selected<?php endif; ?> value="0">否</option>
              </select>
            </div>
          </div>
          <div class="form-group">
            ...
            <div class="col-sm-2">
              <select name="typeid" id="typeid" class="form-control">
                ...
                <?php if(is_array($typename)): $i = 0; $__LIST__ = $typename;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tn): $mod = ($i % 2 );++$i;?><option <?php if($ata["typeid"] == $tn["id"]): ?>selected<?php endif; ?> value="<?php echo ($tn["id"]); ?>"><?php echo ($tn["typename"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
              </select>
            </div>
          </div>
          ...
          <h3>关联选项</h3>
          <div class="attradds">
            <?php if(is_array($al)): $k = 0; $__LIST__ = $al;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$al): $mod = ($k % 2 );++$k;?><div class="form-group">
                <label for="attrval" class="col-sm-2 control-label">属性选项<?php echo ($k); ?></label>
                <div class="col-sm-3">
                  <input type="hidden" name="attr_value_id[]" value="<?php echo ($al["id"]); ?>">
                  <input name="attr_value[]" value="<?php echo ($al["attr_value"]); ?>" class="form-control" type="text" />
                </div>
                <div class="col-sm-1"><span style="cursor:pointer" rek='<?php echo ($al["attr_id"]); ?>' rel='<?php echo ($al["id"]); ?>' class="glyphicon glyphicon-remove delipt"></span></div>
