  ...
  <script type="text/javascript">
	...
	<div class="cont-box" id="cont-box1">
		<?php if(is_array($productlist)): $i = 0; $__LIST__ = $productlist;if( count($__LIST__)==0 ) : echo "暂无商品" ;else: foreach($__LIST__ as $key=>$plt): $mod = ($i % 2 );++$i;?><div class='bt-bot'>		
                  ...
                  <input type="hidden" name="productid" value="<?php echo ($plt["id"]); ?>">
  								<span style="color: rgb(255,51,0); font-size: 24px">￥</span><span data-tid='<?php echo ($plt["id"]); ?>' class="price" data-p="<?php echo ($plt["price"]); ?>" placeholder="<?php echo ($plt["price"]); ?>" style="color: rgb(255,51,0); font-size: 32px"><?php echo ($plt["price"]); ?></span>
  							</div>
  							...
  							<div style="height: 15px;" class="col-xs-12"> </div>
  							<div class="col-xs-8"><a href="<?php echo U('StoreShopIndex/ProductArticle',array('id'=>$plt['id']));?>"><button type="button" class="btn btn-info btn-sm" style="width:70%; font-size: 16px">商品详情</button></a></div>
  							<div class="col-xs-4"><a href="<?php echo U('StoreProductOrder/ProductOrder',array('id'=>$plt['id'],'tid'=>$sae['typeid']));?>"><button type="botton" class="btn btn-warning btn-sm" style="font-size: 16px">立即预订</button></a></div>
  						</div>
  					</tr>
  					<tr>
  						<td></td>
...
<!--cont-box2-->
  <div class="cont-box" id="cont-box2">
    <div class="row" style="margin: 0px ; padding: 20px; border-bottom: 1px solid #ccc;">
      <div class="col-xs-12 h4">地址：<?php echo ($sae["address"]); ?></div>
      <div class="col-xs-12 h4">电话：<?php echo ($sae["tel"]); ?></div>
    ...
    <!-- 简介 -->
    <div class="h4" style="margin: 20px; padding-left: 15px; border-left: 3px solid rgb(50,184,173)"><?php echo ($sae["shopname"]); ?>简介</div>

    <div style="margin: 20px; padding-left: 15px;">
      <?php echo (htmlspecialchars_decode($sae["business_about"])); ?>
    </div>
  </div>
