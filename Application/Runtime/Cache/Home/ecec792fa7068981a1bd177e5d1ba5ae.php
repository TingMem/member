...
<div class="search">
  ...
  <div class="form-group">
        ...
        <select name="t2" class="form-control">
          <option <?php if($keysave["t2"] == 0): ?>selected<?php endif; ?> value="0">全部</option>
          <option <?php if($keysave["t2"] == 1): ?>selected<?php endif; ?> value="1">未付款</option>
          <option <?php if($keysave["t2"] == 2): ?>selected<?php endif; ?> value="2">已付款</option>
          <option <?php if($keysave["t2"] == 3): ?>selected<?php endif; ?> value="3">已使用</option>
        </select>
    <div class="input-group">
      <input class="form-control" type="text" name="search" id="search" value="<?php echo ($keysave["keyword"]); ?>" placeholder="订单号/商品名">
...
<div style="margin:0px 2%;">
  ...
  <ul style="margin:0px;">
    <form id="form2" name="form2" method="post" action="<?php echo U('StoreOrder/OrderDel');?>">
      <ul>
        <table class="table table-bordered" width="96%" style="background:#fff;"  cellpadding="0">
          ...
          <?php if(is_array($ol)): $i = 0; $__LIST__ = $ol;if( count($__LIST__)==0 ) : echo "$msg" ;else: foreach($__LIST__ as $key=>$ol): $mod = ($i % 2 );++$i;?><tr>
              ...
              <td align="center" ><?php echo ($ol["order_id"]); ?></td>
              <td align="center" ><?php echo ($ol["username"]); ?></td>
              <td align="center" ><?php echo ($ol["product_name"]); ?></td>
              <td align="center" ><?php echo ($ol["price"]); ?></td>
              <td align="center" ><?php echo ($ol["num"]); ?></td>
              <td align="center" ><?php echo ($ol["total_price"]); ?></td>
              <td align="center" >
                <?php switch($ol["order_status"]): case "0": ?>未付款<?php break;?>
                    <?php case "1": ?>已付款<?php break;?>
                    <?php case "2": ?>已使用<?php break; endswitch;?>
              </td>
              <td align="center" ><?php echo ($ol["pay_type"]); ?></td>
              <td align="center" ><?php echo (date('Y-m-d H:i:s',$ol["createtime"])); ?></td>
            </tr><?php endforeach; endif; else: echo "$msg" ;endif; ?>
        </table>
       <div style="text-align: center;"><?php echo ($page); ?></div>
        <div style="text-align: center;">
          <input  name="submitbtn" class="btn btn-primary" onclick="return confirm('您确定要删除选中的订单吗？')" value="删除选中" type="submit" />
          <span class="fonts">（请谨慎删除订单，删除后不可恢复）</span>
        </div>
      </ul>
    </form> 
