...
<body>
	...
	<div class="container-fulid">
		<div class="row search-bg" style="margin: 0px">
			<div class="col-xs-2 search-title"><?php echo ($tb['mark']); ?></div>
			<div class="col-xs-8">
...
jQuery(document).ready(function($) {
	...
	layui.use('flow', function(){
	  ...
	  flow.load({
	    ...
	    ,done: function(page, next){ //到达临界点（默认滚动触发），触发下一页
	      ...
	      $.getJSON("<?php echo U('StoreTypeList/PageSplit');?>?typeid="+tid+"&page="+page, function(res){
	        //假设你的列表返回在data集合中
	        ...
	        layui.each(res.data, function(index, item){
	        	...
	        	imgs = str.split(",");
	          lis.push('<div class="row shoplist" style="margin: 0px; margin-bottom: 5px; padding: 10px;"><div class="col-xs-4"><a href="<?php echo U('StoreShopIndex/ShopArticle',array('id'=>'+item.id+'));?>"><img class="shoplistimg" src="'+imgs[0]+'"/></a></div><div class="col-xs-8" style="padding: 3px 30px;"><h4 style="margin: 0px; font-weight: bold;"><a href="<?php echo U("StoreShopIndex/ShopArticle",array("id"=>'+item.id+'));?>">'+item.shopname+'</a></h4><p><a href="<?php echo U("StoreShopIndex/ShopArticle",array("id"=>'+item.id+'));?>">'+item.business_about+'</a></p></div></div>');
	        }); 
	...
	<!-- 下拉加载 End -->
		...
		<div id="list-box">
			<?php if(is_array($shoplist)): $i = 0; $__LIST__ = $shoplist;if( count($__LIST__)==0 ) : echo "$emp" ;else: foreach($__LIST__ as $key=>$slt): $mod = ($i % 2 );++$i;?><input type="hidden" name="typeid" class="typeid" value="<?php echo ($slt["typeid"]); ?>">
				...
				</div><?php endforeach; endif; else: echo "$emp" ;endif; ?>
		</div>
